...
The threat model assumes only `GET /i/*` and `GET /healthz` are publicly reachable.
`POST /upload` and `/admin*` must be fronted by Cloudflare Access. Set up three
applications in **Zero Trust → Access → Applications** on the same hostname, ordered
most-specific first.
...
You can also leave `/i/*` and `/healthz` outside Access entirely. Adding a Bypass
