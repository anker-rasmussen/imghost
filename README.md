...
| T9 | Supply chain | `Cargo.lock` committed; `cargo audit` / `cargo deny` documented below. |
...
├── src/                           main, config, auth, upload, serve, admin, storage
...
│   ├── imghost.env.example
│   └── access-policies.md         walkthrough for the three CF Access apps
└── (legacy portfolio)             index.html, favicon.svg, me.jpg, CNAME, LICENSE
```

## Build & local run

```bash
# Build & lint with strict warnings.
...
# → {"url":"http://localhost:8080/i/<id>.jpg","deduped":false}
...
```bash
sudo install -d /etc/cloudflared /var/lib/imghost
...
Follow `deploy/access-policies.md`. Three apps on `aigf.dev`:
...
Copy the service token Client ID / Client Secret from the dashboard into
...
Run before each deploy. CI hookup is left as a follow-up.
